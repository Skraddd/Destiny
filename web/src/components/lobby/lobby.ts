...
export default class Lobby extends Vue {
    ...
    handleLobbyChange = async function(this: Lobby, result: Result) {
        ...
        for (const member of state.members) {
            ...
            if (member.summoner && !member.summoner.displayName) member.summoner.displayName = summonerName(member.summoner);
        }
        ...
        for (const invite of state.invitations) {
            ...
            if (invite.toSummoner && !invite.toSummoner.displayName) invite.toSummoner.displayName = summonerName(invite.toSummoner);
        }
