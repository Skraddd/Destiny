...
namespace Conduit
{
    ...
    public static class RiftHost
    {
        ...
        public static void Start()
        {
            ...
            {
                SetStatus("Not found");
                Log("[Conduit] Could not locate the 'rift' folder. Set the MIMIC_RIFT_DIR environment variable to its path.");
                return;
            }

            string fileName, arguments;
            if (File.Exists(Path.Combine(riftDir, "dist", "index.js")))
            {
            ...
            {
                SetStatus("Error");
                Log("[Conduit] Could not start Rift: " + e.Message + ". Make sure node.js and yarn are installed and in PATH.");
        ...
        /**
         * Locates the rift folder: MIMIC_RIFT_DIR env var, or walking up from
         * the executable location looking for rift/package.json.
         */
        private static string FindRiftDirectory()
        {
            var env = Environment.GetEnvironmentVariable("MIMIC_RIFT_DIR");
            if (!string.IsNullOrEmpty(env) && File.Exists(Path.Combine(env, "package.json"))) return env;
