...
namespace Conduit
{
    ...
    class MobileConnectionHandler
    {
        ...
        public MobileConnectionHandler(LeagueConnection league, SendMessageDelegate send)
        {
            ...
            this.SendRaw = send;
            this.Send = msg => SendRaw(plainMode ? msg : ("\"" + CryptoHelpers.EncryptAES(key, msg) + "\""));
        }
        ...
        public void HandleMessage(dynamic msg)
        {
            if (key != null)
            {
                try
                {
                    ...
                    this.HandleDestinyMessage(SimpleJson.DeserializeObject(contents));
                }
                catch
                {
                    // Ignore message.
            ...
            // In plain (local) mode, messages arrive as raw JSON arrays without encryption.
            if (plainMode && msg[0] != (long) MobileOpcode.Secret && msg[0] != (long) MobileOpcode.SecretPlain)
            {
                this.HandleDestinyMessage(msg);
                return;
            }

            if (msg[0] == (long) MobileOpcode.SecretPlain)
            {
                // Unencrypted handshake used for local (LAN) connections where the browser
                // has no access to WebCrypto (plain http). Auto-approve without prompting.
                dynamic plainContents = msg[1];
                if (plainContents == null || plainContents["identity"] == null) return;

                this.plainMode = true;
                Persistence.ApproveDevice(plainContents["identity"]);
            ...
            if (msg[0] == (long) MobileOpcode.Secret)
            {
                ...
                {
                    // Auto-approve new devices without showing a prompt.
                    this.key = Convert.FromBase64String((string)contents["secret"]);
         ...
         */
        private async void HandleDestinyMessage(dynamic msg)
        {
            if (!(msg is JsonArray)) return;
    ...
    enum MobileOpcode : long
    {
        ...
        // Mobile -> Conduit, sends an unencrypted identification payload (local/LAN mode).
        SecretPlain = 10,
