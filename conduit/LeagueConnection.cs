...
namespace Conduit
{
    ...
    class LeagueConnection
    {
        ...
        private void HandleMessage(object sender, MessageEventArgs args)
        {
            ...
            var eventPath = (string) ev["uri"];
            var eventData = ev["eventType"] == "Delete" ? null : ev["data"];

            // Auto-accept ready checks if the user enabled the option in the tray menu.
            // This is handled here (and not via Observe) so it keeps working even when
            // listeners are cleared on hub reconnects.
            if (eventPath == "/lol-matchmaking/v1/ready-check" && Persistence.GetAutoAccept())
            {
                try
                {
                    if (eventData != null && (string) eventData["state"] == "InProgress" && (string) eventData["playerResponse"] == "None")
                    {
                        var ignored = Request("POST", "/lol-matchmaking/v1/ready-check/accept", null);
                        DebugLogger.Global.WriteMessage("Auto-accepted ready check.");
                    }
                }
                catch (Exception e)
                {
                    DebugLogger.Global.WriteError("Auto-accept failed: " + e.Message);
                }
            }
