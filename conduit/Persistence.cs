...
namespace Conduit
{
    ...
    class Persistence
    {
        public static string DATA_DIRECTORY = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "DestinyLocal");
        ...
        private static readonly string AUTO_ACCEPT_PATH = Path.Combine(DATA_DIRECTORY, "autoaccept");
         ...
         */
        /**
         * Returns whether the user enabled automatically accepting ready checks.
         */
        public static bool GetAutoAccept()
        {
            return File.Exists(AUTO_ACCEPT_PATH);
        }

        /**
         * Enables or disables automatically accepting ready checks.
         */
        public static void SetAutoAccept(bool enabled)
        {
            try
            {
                if (enabled) File.WriteAllText(AUTO_ACCEPT_PATH, "1");
                else if (File.Exists(AUTO_ACCEPT_PATH)) File.Delete(AUTO_ACCEPT_PATH);
            }
            catch (Exception e)
            {
                DebugLogger.Global.WriteError("Could not persist auto-accept setting: " + e.Message);
            }
        }
        ...
        /**
         * Toggles whether or not destiny should launch at startup.
         */
