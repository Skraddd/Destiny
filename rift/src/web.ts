...
// RIFT_WEB_DIR or in ../web/dist), serve it. This makes it possible to run Destiny
...
// Candidate locations for the built web interface. When packaged as a
// standalone exe (pkg), __dirname points inside the snapshot, so paths
// relative to the real executable are also tried.
const webDirCandidates = [
    process.env.RIFT_WEB_DIR,
    path.join(path.dirname(process.execPath), "..", "web", "dist"), // {app}\rift\rift.exe -> {app}\web\dist
    path.join(path.dirname(process.execPath), "web", "dist"),       // rift.exe next to a web folder
    path.join(__dirname, "..", "..", "web", "dist")                  // running from source/dist with node
].filter(x => !!x) as string[];

const webDir = webDirCandidates.filter(x => fs.existsSync(path.join(x, "index.html")))[0];
if (webDir) {
